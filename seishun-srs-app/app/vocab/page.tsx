...
export default async function VocabPage({ searchParams }: PageProps) {
  ...
  } else if (userId && status === "learning") {
    where.progress = { some: { userId } };
  } else if (userId && status === "due") {
    where.progress = { some: { userId, nextReviewDate: { lte: now } } };
  }
  ...
  const [entries, total] = await Promise.all([
    prisma.vocabEntry.findMany({
      ...
      select: {
        ...
        partsOfSpeech: true,
        progress: userId
          ? { where: { userId }, select: { nextReviewDate: true, interval: true, repetitions: true }, take: 1 }
          : false,
      },
    }),
    ...
    <div className="min-h-screen px-4 py-8 max-w-5xl mx-auto">
      ...
      {entries.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-8">
          {entries.map((entry) => {
            const prog = Array.isArray(entry.progress) ? entry.progress[0] ?? null : null;
            const srsStatus = getSRSStatus(prog, now);
            return (
              <Link
                key={entry.id}
                href={`/vocab/${entry.id}`}
...
function Pagination({
  ...
  return (
    <div className="flex items-center justify-center gap-1 pb-8">
      {page > 1 && (
        <Link href={makeHref(page - 1)} className="pager-btn">←</Link>
      )}
      {pages.map((p, i) =>
        p === "…" ? (
